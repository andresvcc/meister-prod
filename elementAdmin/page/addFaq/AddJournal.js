...
function AddFaq({ useSocketHook }) {
  ...
  const saveToDatabase = async (dataForm) => {
    ...
    await emit('faqJournalOrder', { value: [...new Set([value.title, ...order])] });
    await emit('faqJournal', { key: value.title, value: { ...value, photo } });
  };

  useEffect(() => {
    if (socket) {
      socket.on('faqJournal', (data) => router.push('/admin/faq/'));
    }
  }, [socket]);
    ...
    <Div width="100%" row horizontal="left" vertical="top">
      ...
      >
        ...
        <Div row width="120px" horizontal="at">
          ...
          <Div onClick={() => router.push('/admin/faq')}>
