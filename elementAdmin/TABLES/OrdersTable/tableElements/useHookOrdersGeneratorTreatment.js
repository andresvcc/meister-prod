...
import SweetTreatment from './SweetTreatment';
...
const format = (val) => {
  ...
  const total = [...val.products.map((val) => (val?.price * val?.qty) + (val?.price * val?.qty) * val.TVA), 0, 0].reduce((a, b) => a + b) + (val.shippingCost || 0);
...
const useHookOrdersGenerator = ({ useSocketHook }) => {
  ...
  const ordersArr = useMemo(() => Object.values(list).filter(({ status }) => status === 'treatment'), [list]);

  const [listBinding, setList] = useState(list);
  ...
  useEffect(() => {
    if (orders !== undefined && orders.list && users) {
      // get all data
      ...
      const newOrdersData = ordersArr.map(({
        ...
        products: users[user]?.billings[idBilling]?.products.map((product, i) => ({ ...product, providers: productList[product.name].Provider, i })),
  ...
  useEffect(() => {
    const newOrdersData = Object.values(listBinding).filter(({ status }) => status === 'treatment').map(({
      indexOrder, dateOrder, idBilling, indexBilling, payCode, payDate, payService, pay, user
    }, i) => ({
      indexOrder: i,
      idOrder: `${i}`,
