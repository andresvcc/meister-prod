...
const useHookOrdersGenerator = ({ useSocketHook }) => {
  ...
  const { list = {} } = orders || {};
  const ordersArr = Object.values(list).filter(({ status }) => status === 'reception');
      ...
      const rowData = newOrdersData.map(({
        ...
        <Div width="100%">
          <Div horizontal="right" style={{ userSelect: 'all', WebkitUserSelect: 'all', MozUserSelect: 'all' }}>
            <p style={{ fontSize: '16px', fontWeight: 'bolder' }}>{`${([...[...products].map((val) => val.price + (val.price * 0.16)), 0, 0].reduce(reducer) + parseFloat(shippingCost, 10)).toFixed(2)} ${currency}`}</p>
          </Div>
        </Div>,
      ]);
