...
const useHookOrdersGenerator = ({ useSocketHook }) => {
  ...
  const { list = {} } = orders || {};
  const ordersArr = useMemo(() => Object.values(list).filter(({ status }) => status === 'delivered'), [orders]);
  ...
  useEffect(() => {
    ...
    // console.log('if entries', orders !== undefined && !!orders.list && !!users && ordersArr.length >= 0);
  ...
  }, [ordersArr, width, users]);

  useEffect(() => console.log('ordersData', ordersArr), [ordersArr]);
